...
class Heming:
    '''Heming coder/decoder class.'''
    config = None
    log = None

    def __init__(self):
        self.log = logger.Logger("heming.log")

    def code(self, data, code_mode):
        '''Coding/decoding with heming code, use 1 to code and 0 to decode.'''
        self.config = configparser.ConfigParser()
        self.config.read("settings.ini")
        lenght = 0

        try:
            lenght = int(self.config["Heming"]["Lenght"])
        except KeyError:
            print("\nПараметр не обнаружен!")
            self.log.error("Не обнаружен параметр lenght в конфиге.")
        except ValueError:
            print("\nНеверный тип данных!")
            self.log.error("Параметр lenght содержит неверный тип данных.")
